...
for order=min_order:max_order
    ...
    for alpha = min_alpha:max_alpha
        ...
        %{
         ...
         plot_sound_field(T,X,Y,Z,x0,conf)
         hold on
         figure;
         insideCircle = x0(:,1).^2 + x0(:,2).^2 <= yl_r.^2;
         boundaryPoints = (abs(x0(:,1).^2 + x0(:,2).^2 - yl_r.^2) < 1.2);
         upper_z = x0(:,3) > 0;
         lower_z = x0(:,3) < 0;
         upper = (~boundaryPoints) & upper_z;
         lower = (~boundaryPoints) & lower_z;
         plot(x0(boundaryPoints,1),x0(boundaryPoints,2),'o', ...
             'MarkerFaceColor','k', ...
             'MarkerEdgeColor','k', ...
             'MarkerSize',4);
         plot(x0(upper,1),x0(upper,2),'o', ...
             'MarkerFaceColor','r', ...
             'MarkerEdgeColor','r', ...
             'MarkerSize',4);
         plot(x0(lower,1),x0(lower,2),'o', ...
             'MarkerFaceColor','b', ...
             'MarkerEdgeColor','b', ...
             'MarkerSize',4);
         title(['Plane-wave with Lame Array α = ',num2str(alpha)]);
         figset;
         %savefig([num2str(f),'Hz_alpha_',num2str(alpha),'_y_[',num2str(y(1)),',',num2str(y(2)),',',num2str(y(3)),'].fig']);
        %}
    end  
end
...
%%
T_sum_all_1_46_Lame4 = T_sum_all_1_5+T_sum_all_6_10+T_sum_all_11_15+T_sum_all_16_20+T_sum_all_21_25+T_sum_all_26_30+T_sum_all_31_35+T_sum_all_36_40+T_sum_all_41_46;

T_lame4_center = T_sum_all_1_46_Lame4(32,32,1,1,:);
T_lame4_center = squeeze(T_lame4_center);
%%
figure;
subplot(2,1,1);
plot(t2, P2);
...
%%
figure;
scatter3(yl_x, yl_y, yl_z, 10, 'filled'); % 50はマーカーサイズ、'filled'で塗りつぶし
xlabel('X [m]');
ylabel('Y [m]');
zlabel('Z [m]');
...
figure;
for t = 1:length(t1)
    surf(xx1, yy1, real(T_sum_all_1_46_Lame4(:,:,:,:,t)));
    xlim([-1.6 1.6]);
    ylim([-1.6 1.6]);
    clim([min(real(T_sum_all_1_46_Lame4(32,32,1,1,:))) max(real(T_sum_all_1_46_Lame4(32,32,1,1,:)))]);
    title(sprintf('Pressure Field\n t=%.4f s', t1(t)));
    view(2);
    shading interp;
    axis square;
...
ylim([-1.6 1.6]);
%clim([-1 500]);
clim([min(real(T_sum_all_1_46_Lame4(81,81,1,1,:))) max(real(T_sum_all_1_46_Lame4(81,81,1,1,:)))]);
title(sprintf('Pressure Field\n t=%.4f s', t1(t)));
view(2);
shading interp;
axis square;
